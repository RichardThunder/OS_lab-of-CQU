...
struct tcb *p = g_task_running;
  ...
  { //如果是task0在运行，则强制调度
    ...
    { //每次定时器中断：g_task_running->estcpu++，task0除外！
      ...
      if (g_timer_ticks % HZ == 0) //每秒刷新定时器
      {
       
        int nready ;//表示处于就绪状态的线程个数，task0除外！
        nready=0;
        fixedpt ratio;
        p=g_task_head;
        while (p != NULL)
        {
          ...
          p->estcpu = fixedpt_add(fixedpt_mul(ratio, p->estcpu), fixedpt_fromint(p->nice));
          p = p->next;
        }
        //	g_load_avg：表示系统的平均负荷 初值为0 每秒钟更新一次
