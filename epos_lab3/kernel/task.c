...
struct tcb *select=g_task_running;
    struct tcb *p =g_task_head;
     //先计算各个线程的动态优先级priority（task0除外）
        select = g_task_head;   
        while(select!= NULL)
         { 
                select->priority = PRI_USER_MAX- fixedpt_toint(fixedpt_div(select->estcpu, fixedpt_fromint(1))) - select->nice*2; 
                 select=select->next;
         }
        ...
        do {          
            p=p->next;
               ...
               {
                if(p->priority > select->priority||select->tid==0)       
                select=p;//选择等待队列中优先级最高的线程，task0除外   
              }
            } while(1);
...
{ 
    if(tid ==0)//如果参数tid=0，表示获取/设置当前线程的nice值，而不是task0！
...
int setpriority(int tid, int prio)
{
    if(tid==0)
    {
    g_task_running->nice=prio-NZERO;//如果参数tid=0，表示获取/设置当前线程的nice值，而不是task0！
    return 0;
    }   
    ...
    if(prio>=0 && prio<=2*NZERO-1 )//prio必须在[0,2*NZERO-1]内
    {tsk->nice=prio-NZERO;  //把线程tid的nice设为(prio-NZERO)
    return 0;
    }
    else return -1;
